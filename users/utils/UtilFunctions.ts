...
export default class UtilFunctions {
    ...
    public static defineResponseStatus = (
        httpStatus: number
    ): ResponseStatus => {
    ...
    public static signTokenAndStoreInCookies = async (
        ...
        const token = UtilFunctions.signToken(
            jwtPayload,
        ...
        process
            .on("unhandledRejection", (reason, p) => {
                ...
                server.close((err: any) => {
                    ...
                    sequelize.close().then(() => {
                        UtilFunctions.errorLogger.error(
                            "Sequelize connection disconnected"
                        );
                        process.exit(0);
                    });
                });
            ...
            .on("uncaughtException", (err) => {
                ...
                server.close((err: any) => {
                    ...
                    sequelize.close().then(() => {
                        UtilFunctions.informationalLogger.info(
                            "Sequelize connection disconnected"
                        );
                        process.exit(0);
                    });
